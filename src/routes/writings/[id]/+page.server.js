...
export async function load({ locals, params }) {
    try {
        ...
        const writing = await locals.db.prepare(
            'SELECT w.id, w.user_id, w.prompt_id, w.title, w.content, w.word_count, w.ai_assisted, w.visibility, w.status, w.created_at, w.updated_at, w.locale, w.category, p.prompt_text, p.category as prompt_category FROM writings w LEFT JOIN writing_prompts p ON w.prompt_id = p.id WHERE w.id = ?'
