<script>
    ...
    async function toggleNavmesh() {
        ...
        navVisible = true;
        mapInstance.flyTo([20.60135, -105.23894], 16, { duration: 1.5 });
        drawDynamicGrid();
        drawMovementRange();
    }
    ...
    onMount(async () => {
        ...
        if (pvPts.length > 3) {
            ...
            mapInstance.setView([20.60135, -105.23894], 16);
        }
    });
...
{#if data.myPlayer}
    <a href="/games/booty/arbooty" style="display:block;background:var(--surface);border:2px solid var(--accent);border-radius:var(--radius);padding:1.5rem;margin-bottom:2rem;text-decoration:none;color:var(--text);">
