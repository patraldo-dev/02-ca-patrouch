<script>
    ...
    // i18n strings for card (standalone page, no store context)
    const i18n = {
        en: {
            words: 'words',
            by: 'by',
            ...
            share_text: (t, a, w) => `"${t}" by ${a} — ${w} words on patrouch.ca`
        },
        es: {
            words: 'palabras',
            by: 'por',
            ...
            share_text: (t, a, w) => `"${t}" por ${a} — ${w} palabras en patrouch.ca`
        },
        fr: {
            words: 'mots',
            by: 'par',
            ...
            share_text: (t, a, w) => `"${t}" par ${a} — ${w} mots sur patrouch.ca`
        }
    };
    ...
    const dateStr = new Date(createdAt.replace(' ', 'T')).toLocaleDateString(localeMap[locale] || 'en-US', {
        month: 'long', day: 'numeric', year: 'numeric'
    });

    const shareUrl = typeof window !== 'undefined' ? window.location.href : `https://patrouch.ca/card/${id}`;
    const shareText = t.share_text(title, author, wordCount);
    ...
    function shareMastodon() {
        const tootText = `${shareText}\n\n${shareUrl}`;
        window.open(`https://mastodon.social/share?text=${encodeURIComponent(tootText)}`, '_blank');
    }
...
<div class="card-container">
    <div class="card" id="writing-card">
        <div class="card-header">
            ...
            {#if aiAssisted}
                <span class="ai-badge">{t.ai_assisted}</span>
            {/if}
        </div>

        <h1 class="card-title">{title}</h1>
        <p class="card-author">{t.by} <strong>{author}</strong></p>
        ...
        <div class="card-footer">
            <span class="card-meta">{wordCount} {t.words}</span>
    ...
    <div class="actions">
        ...
        <div class="share-buttons">
            ...
            <button class="share-btn mastodon" onclick={shareMastodon} title="Mastodon">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M23.268 5.313c-.35-2.578-2.617-4.61-5.304-5.004C17.51.242 15.792 0 11.813 0h-.03c-3.98 0-4.835.242-5.288.309C3.882.692 1.496 2.518.917 5.127.64 6.412.61 7.837.661 9.143c.074 1.874.088 3.745.26 5.611.118 1.24.325 3.085.576 4.417.283 1.32.759 2.135 1.38 2.895.67.82 1.563 1.29 2.642 1.42.49.058.99.088 1.49.088h.03c.51 0 1.02-.03 1.53-.088 1.08-.13 1.972-.6 2.642-1.42.62-.76 1.097-1.575 1.38-2.895.25-1.332.458-3.177.576-4.417.172-1.866.186-3.737.26-5.611.05-1.306.02-2.731-.26-4.032zm-2.85 12.99c-.16.84-.42 1.5-.78 2.01-.48.66-1.14 1.02-1.98 1.11-.54.06-1.08.09-1.62.09h-.03c-.54 0-1.08-.03-1.62-.09-.84-.09-1.5-.45-1.98-1.11-.36-.51-.62-1.17-.78-2.01-.12-.6-.18-1.2-.24-1.8-.06-.6-.09-1.2-.09-1.8 0-.6.03-1.2.09-1.8.06-.6.12-1.2.24-1.8.16-.84.42-1.5.78-2.01.48-.66 1.14-1.02 1.98-1.11.54-.06 1.08-.09 1.62-.09h.03c.54 0 1.08.03 1.62.09.84.09 1.5.45 1.98 1.11.36.51.62 1.17.78 2.01.12.6.18 1.2.24 1.8.06.6.09 1.2.09 1.8 0 .6-.03 1.2-.09 1.8-.06.6-.12 1.2-.24 1.8zm-3.39-4.59v2.79h-1.56v-2.79c0-.78-.39-1.17-1.17-1.17-.78 0-1.17.39-1.17 1.17v2.79H11.56v-2.79c0-.78-.39-1.17-1.17-1.17-.78 0-1.17.39-1.17 1.17v2.79H7.66V9.9h1.56v.78c.39-.51.9-.78 1.56-.78.66 0 1.17.27 1.56.78.39-.51.9-.78 1.56-.78.66 0 1.17.27 1.56.78v-.78h1.56v4.2z"/></svg>
            </button>
        ...
        <a href="/writings/{id}" class="back-link">{t.read_full}</a>
    </div>
</div>
