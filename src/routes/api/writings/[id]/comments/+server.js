...
export async function GET({ locals, params, url }) {
    ...
    // Check writing exists and is published
...
export async function POST({ locals, params, request, platform }) {
    if (!locals.user) return json({ error: 'Unauthorized' }, { status: 401 });
    ...
    if (parent_id) {
        ...
        // Verify parent comment exists on this writing
        ...
        try {
            ...
            if (ai) {
                ...
                if (decision.startsWith('approve')) {
                    ...
                    status = 'rejected';
                    // Still store but mark rejected
                }
            } else {
                status = 'approved'; // fallback if no AI binding
            }
        } catch (e) {
            console.error('Moderation error:', e);
            status = 'approved'; // fail open
        }
    }
