...
// Booty Bot Autonomous Cron
// POST to /api/bottlequest/bot-ai-cron to trigger
...
export async function POST({ request, platform }) {
    ...
    try {
        // 1. Fetch recent writings (last 3 days)
        ...
        `).all();
        const writingsText = (writings || []).map(w =>
            `[${w.display_name || w.username}] "${w.title}": ${(w.content || '').slice(0, 300)}`
        ).join('\n\n');

        // 2. Fetch active bottles
        ...
        // 4. Fetch active narrator events
        ...
        for (const bot of bots) {
            // Check if hijacked - if so, skip autonomous decision
            if (bot.hijacked_by && new Date(bot.hijacked_until) > new Date()) {
                results.push({ bot: bot.name, action: 'hijacked', by: bot.hijacked_by });
                continue;
            }

            // Check narrator events affecting this bot
            const stormEvent = events.find(e =>
                e.event_type === 'storm' && (!e.target_players || e.target_players.includes(bot.id))
            );
            if (stormEvent) {
            ...
            const paralysisEvent = events.find(e =>
                e.event_type === 'paralysis' && (!e.target_players || e.target_players.includes(bot.id))
            );
            if (paralysisEvent) {
...
Decide your action. Reply with ONLY valid JSON:
{
  ...
  "reason": "<why you did this, max 80 chars>"
            ...
            // 9. Execute decision with Asimov enforcement
            ...
            if (decision.action === 'move' && decision.target_lat && decision.target_lon) {
                ...
                if (cost > 0 && bot.beans >= cost) {
                    await db.prepare('UPDATE bq_booty_bots SET lat = ?, lon = ?, beans = beans - ? WHERE id = ?')
                        .bind(decision.target_lat, decision.target_lon, cost, bot.id).run();
                    result.moved = true;
                    result.cost = cost;
                } else {
                    result.moved = false;
            ...
            if (decision.action === 'steal_beans' && decision.target_player) {
                ...
                if (target && target.fuel > 0) {
                    ...
                    const steal = Math.min(maxSteal, 5); // Also cap at 5 flat
                    if (steal > 0) {
            ...
            if (decision.action === 'comment' && decision.comment) {
                // Store bot comment (could show in narrator events or bot section)
        ...
        // 10. Check bot-vs-bot combat (proximity < 0.001°)
