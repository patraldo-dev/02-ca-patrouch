<script>
    ...
    const steps = $derived([
        ...
        {
            title: $t('onboarding.step2b_title'),
            body: $t('onboarding.step2b_body'),
            icon: '🏴‍☠️'
        },
        {
...
<div class="onboarding-overlay">
    <div class="onboarding-card" class:animate={animating}>
        ...
        <div class="step-content">
            ...
            <h2>{steps[step].title} {#if steps[step].memberOnly}<span class="member-badge">{$t('onboarding.member_only')}</span>{/if}</h2>
            <p>{steps[step].body}</p>
        </div>
