...
async function POST({ request, locals, platform }) {
  ...
  try {
    const { amount: amount2, message } = await request.json();
    if (!amount2 || amount2 < 1 || amount2 > 50) {
      return json({ error: "Amount must be 1-50" }, { status: 400 });
    }
    const player2 = await db.prepare("SELECT id FROM bq_players WHERE username = ?").bind(locals.user.username).first();
    if (!player2) return json({ error: "No player found" }, { status: 404 });
    const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const existing = await db.prepare(
      `SELECT id FROM bq_fuel_requests WHERE player_id = ? AND status = 'open' AND date(created_at) = ?`
    ).bind(player2.id, today).first();
    ...
    ).bind(id, player2.id, amount2, (message || "").slice(0, 140)).run();
