...
async function POST({ request, platform, locals }) {
  ...
  try {
    ...
    if (action === "move") {
      ...
      await db.prepare("UPDATE bq_booty_bots SET lat = ?, lon = ?, beans = beans - ? WHERE id = ?").bind(targetLat, targetLon, cost, botId).run();
      return json({ success: true, bot: bot.username, moved: true, cost, newLat: targetLat, newLon: targetLon });
    }
    if (action === "capture") {
      const bot = await db.prepare("SELECT * FROM bq_booty_bots WHERE id = ?").bind(botId).first();
      if (!bot) return json({ error: "Bot not found" }, { status: 404 });
      await db.prepare("UPDATE bq_booty_bots SET captured_bottles = captured_bottles + 1, beans = beans + 50 WHERE id = ?").bind(botId).run();
...
async function PATCH({ request, locals, platform }) {
  ...
  try {
    ...
    const bot = await db.prepare("SELECT * FROM bq_booty_bots WHERE id = ?").bind(botId).first();
    if (!bot) return json({ error: "Bot not found" }, { status: 404 });
    if (bot.hijacked_by && bot.hijacked_until && new Date(bot.hijacked_until) > /* @__PURE__ */ new Date()) {
    ...
    await db.prepare("UPDATE bq_booty_bots SET hijacked_by = ?, hijacked_until = ? WHERE id = ?").bind(locals.user.username, until, botId).run();
