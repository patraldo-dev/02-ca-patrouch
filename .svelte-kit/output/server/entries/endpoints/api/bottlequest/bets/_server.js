...
function calcOdds(distKm) {
  ...
  if (distKm <= 1e3) return 2 + distKm / 500;
  return 5 + distKm / 2e3;
}
async function POST({ request, locals, platform }) {
  ...
  try {
    ...
    return json({
      odds,
      potential_win: Math.floor(amount * odds * 0.95),
      // 5% bot commission on winnings
...
async function GET({ url, locals, platform }) {
  const db = platform?.env?.DB_book;
  if (!db) return json({ error: "No DB" }, { status: 500 });
  try {
...
async function PUT({ request, platform }) {
  const db = platform?.env?.DB_book;
  if (!db) return json({ error: "No DB" }, { status: 500 });
  try {
