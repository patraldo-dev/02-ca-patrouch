...
async function POST({ request, locals, platform }) {
  ...
  try {
    ...
    if (provider === "kokoro") {
      const row2 = await locals.db.prepare("SELECT deepinfra_key_encrypted FROM users WHERE id = ?").bind(user.id).first();
      if (!row2?.deepinfra_key_encrypted) {
        return json({ error: "no_hf_key" }, { status: 503 });
      }
      const hfKey = decryptUserKey(row2.deepinfra_key_encrypted, user.id);
      const resp = await fetch("https://api.deepinfra.com/v1/audio/speech", {
        ...
        body: JSON.stringify({ model: "hexgrad/Kokoro-82M", input: text.trim().slice(0, 5e3) })
      });
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        return json({ error: err.error || `Kokoro TTS failed (${resp.status})` }, { status: resp.status });
      }
      const audioBuffer2 = await resp.arrayBuffer();
      const base642 = btoa(String.fromCharCode(...new Uint8Array(audioBuffer2)));
      return json({ audio: base642, format: "wav", provider: "kokoro" });
    }
