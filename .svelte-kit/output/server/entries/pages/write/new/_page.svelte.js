...
function _page($$payload, $$props) {
  ...
  $$payload.out.push(`<div class="editor-page svelte-p7m09"><button class="back-link svelte-p7m09">${escape_html(store_get($$store_subs ??= {}, "$t", t)("write.editor.back"))}</button> <h1 class="editor-heading svelte-p7m09">${escape_html(store_get($$store_subs ??= {}, "$t", t)("write.editor.heading"))}</h1> `);
  if (data.prompt) {
    $$payload.out.push("<!--[-->");
    $$payload.out.push(`<div class="prompt-context svelte-p7m09"><span class="prompt-tag svelte-p7m09">${escape_html(store_get($$store_subs ??= {}, "$t", t)("write.editor.from_prompt"))}</span> <p class="prompt-text svelte-p7m09">${escape_html(data.prompt.prompt_text)}</p> <span class="prompt-cat svelte-p7m09">${escape_html(store_get($$store_subs ??= {}, "$t", t)(catLabel(data.prompt.category)))}</span></div>`);
  } else {
    $$payload.out.push("<!--[!-->");
  }
  $$payload.out.push(`<!--]--> `);
